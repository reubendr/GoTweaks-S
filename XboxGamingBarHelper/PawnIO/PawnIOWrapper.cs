...
namespace XboxGamingBarHelper.PawnIO
{
    ...
    public class PawnIOWrapper : IDisposable
    {
        ...
        public bool Connect()
        {
            ...
            try
            {
                ...
                if (_rawHandle == IntPtr.Zero || _rawHandle.ToInt64() == -1)
                {
                    ...
                    Logger.Error($"Failed to open PawnIO device. Error code: {error}");
                    _rawHandle = IntPtr.Zero;
                    return false;
                }

                Logger.Info("Connected to PawnIO driver");
                return true;
            }
            catch (Exception ex)
            {
                Logger.Error($"Exception connecting to PawnIO: {ex.Message}");
                return false;
            }
        }
        ...
        public bool LoadModuleFromResource(Assembly assembly, string resourceName)
        {
            if (!IsConnected)
            {
                Logger.Error("Cannot load module: not connected to PawnIO");
                return false;
            }
            ...
            try
            {
                using (Stream stream = assembly.GetManifestResourceStream(resourceName))
                {
                    if (stream == null)
                    {
                        ...
                        Logger.Info($"Available resources ({available.Length}): {string.Join(", ", available)}");
                        return false;
                    }
                    ...
                    using (MemoryStream memoryStream = new MemoryStream())
                    {
                        ...
                        Logger.Info($"Loaded embedded resource {resourceName} ({moduleData.Length} bytes)");
            ...
            {
                Logger.Error($"Exception loading module from resource: {ex.Message}");
                return false;
            }
        }
        ...
        public bool LoadModule(string modulePath)
        {
            ...
            {
                Logger.Error($"Exception loading module: {ex.Message}");
                return false;
            }
        }
        ...
        public bool LoadModuleFromBytes(byte[] moduleData, string sourceName = "bytes")
        {
            ...
            try
            {
                Logger.Info($"Loading PawnIO module from {sourceName} ({moduleData.Length} bytes)");
                ...
                else
                {
                    ...
                    Logger.Error($"Failed to load module. Win32 error: {error}, HRESULT: 0x{hr:X8}");
                    return false;
                }
            }
            catch (Exception ex)
        ...
        public bool ExecuteFunction(string functionName, ulong[] inputArgs, ulong[] outputArgs)
        {
            if (!IsModuleLoaded)
            {
                Logger.Error("Cannot execute function: no module loaded");
                return false;
            }
            ...
            try
            {
                ...
                if (result)
                {
                    ...
                    Logger.Debug($"Executed {functionName}: in={inSize}, out={bytesReturned / 8}");
                    return true;
                }
                else
                {
                    int error = Marshal.GetLastWin32Error();
                    Logger.Error($"pawnio_execute({functionName}) failed. Win32 error: {error}");
                    return false;
                }
            }
            catch (Exception ex)
            {
                Logger.Error($"Exception executing function {functionName}: {ex.Message}");
                return false;
            }
        }
        ...
        public void Disconnect()
        {
            ...
            if (_safeHandle != null && !_safeHandle.IsInvalid)
            {
                ...
                {
                    _safeHandle.Close();
                    Logger.Info("Disconnected from PawnIO driver");
                }
                catch (Exception ex)
                {
                    Logger.Warn($"Error closing PawnIO safe handle: {ex.Message}");
                }
                _safeHandle = null;
            }
            ...
            if (_rawHandle != IntPtr.Zero && _rawHandle.ToInt64() != -1)
            {
                try
                {
                    ...
                    Logger.Info("Closed PawnIO raw handle");
                }
                catch (Exception ex)
                {
                    Logger.Warn($"Error closing PawnIO raw handle: {ex.Message}");
                }
                _rawHandle = IntPtr.Zero;
            }
        }
